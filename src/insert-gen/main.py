...
def main():
    ...
    create_sql_is_empty = (
        not exists(create_sql_file) or stat(create_sql_file).st_size == 0
    )
    ...
    if create_sql_is_empty or force and not create_sql_is_empty:
        ...
        print(
            f"File {create_sql_file} is not empty and -f flag is not provided. Skipping..."
        )

    insert_sql_file = join(out_dir, "insert.sql")
    insert_sql_is_empty = (
        not exists(insert_sql_file) or stat(insert_sql_file).st_size == 0
    )
    ...
    if insert_sql_is_empty or force and not insert_sql_is_empty:
        ...
        print(
            f"File {insert_sql_file} is not empty and -f flag is not provided. Skipping..."
        )
