...
from generators.text import PhoneNumberGenerator, EmailGenerator, EmploymentStatusGenerator, get_passport_code, get_driver_license_code
...
def seed_root(context: Context):
    context.shared_unique_generators["phone_number"] = UniqueGenerator(gen=PhoneNumberGenerator())
    context.shared_unique_generators["email"] = UniqueGenerator(gen=EmailGenerator())
    context.shared_unique_generators["passport_code"] = UniqueGenerator(gen=SimpleNamespace(get=get_passport_code))
    context.shared_unique_generators["driver_license_code"] = UniqueGenerator(gen=SimpleNamespace(get=get_driver_license_code))
    ...
    with open("datasets/locations.txt") as f:
        ...
        context.locations.append(models.Location(location_id=location_id, name=location_name))
...
def seed_candidate(context: Context):
    ...
    phone_number: models.PhoneNumber | None = next(filter(lambda pn: pn.phone_number == phone_number_value, context.phone_numbers), None)

    if phone_number is None:
        phone_number_id = get_random_uuid()
        phone_number = models.PhoneNumber(phone_number_id=phone_number_id, phone_number=phone_number_value)
        context.phone_numbers.append(phone_number)

    email = context.shared_unique_generators["email"].get_unique(first_name=first_name, last_name=last_name)
    ...
    driver_license_code = context.shared_unique_generators["driver_license_code"].get_unique()
...
def seed_vacancy(vacancy_data: dict[str, Any], context: Context) -> models.Vacancy:
    ...
    vacancy_category: models.VacancyCategory | None = next(filter(lambda c: c.name == vacancy_category_name, context.vacancy_categories), None)

    if vacancy_category is None:
        vacancy_category_id = get_random_uuid()
        vacancy_category = models.VacancyCategory(vacancy_category_id=vacancy_category_id, name=vacancy_category_name)
...
def seed_company(company_data: dict[str, Any], context: Context) -> models.Company:
    ...
    location: models.Location | None = next(filter(lambda l: l.name == location_name, context.locations), None)
    ...
    production_branch: models.ProductionBranch | None = next(filter(lambda pb: pb.name == production_branch_name, context.production_branches), None)

    if production_branch is None:
        production_branch_id = get_random_uuid()
        production_branch = models.ProductionBranch(production_branch_id=production_branch_id, name=production_branch_name)
